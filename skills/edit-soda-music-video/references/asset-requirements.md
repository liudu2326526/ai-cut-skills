...
需要自动匹配时，执行 Skill 的模型直接用 Read 工具查看图片原图和视频缩略帧，为每个素材补充一段 `description`。检索只使用这段 description；不调用素材理解/匹配脚本，不生成或保存 `keywords`、`recommended_usage` 或向量。文件类型、布局、黑边、渠道和品牌安全区仍由规则校验。
