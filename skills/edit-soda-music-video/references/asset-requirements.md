...
需要自动匹配时，可对图片和视频运行 `understand-assets`，让多模态模型为每个素材补充一段 `content_understanding.description`。检索只使用这段 description；不生成或保存 `keywords`、`recommended_usage` 或向量。文件类型、布局、黑边、渠道和品牌安全区仍由规则校验。

所有素材不得在渲染器中生成黑条、黑框或半透明黑色承托层。视频素材若自身带有跨多帧稳定的固定黑边，预检必须报错；自然画面中的黑色内容不按黑边处理。

素材不必避让字幕安全区，可以覆盖到字幕所在空间；但素材可见内容不得进入左上 logo 保护区或底部警示语保护区。渲染器必须先保持时间轴指定的素材尺寸并尝试平移避让；只有平移仍无法避让时，才按照可用区计算不遮挡 logo/警示语的最大等比尺寸，不设置固定缩放上限或下限。透明图片仍需按 alpha 可见边界判断；视频、无透明图片以及 Remotion 动效也必须限制在同一素材可用区。所有素材叠加完成后绘制字幕与 CTA，再叠加 logo，最后绘制警示语。字幕图层始终高于素材图层，logo 高于素材/字幕/CTA，警示语为最终最高层。
