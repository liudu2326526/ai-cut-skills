...
素材不必避让字幕安全区，可以覆盖到字幕所在空间；但素材可见内容不得进入左上 logo 保护区或底部警示语保护区。渲染器必须先保持时间轴指定的素材尺寸并尝试平移避让；只有平移仍无法避让时，才按 `visual_policy.minimum_material_scale` 做有限等比缩小，默认最低为原尺寸的 `85%`。超过这个缩放下限仍无法避让时必须报错，不能静默大幅缩放。透明图片仍需按 alpha 可见边界判断；视频、无透明图片以及 Remotion 动效也必须限制在同一素材可用区。所有素材叠加完成后绘制字幕与 CTA，再叠加 logo，最后绘制警示语。字幕图层始终高于素材图层，logo 高于素材/字幕/CTA，警示语为最终最高层。
