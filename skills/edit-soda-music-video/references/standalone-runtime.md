...
Manifest 默认写入工作区的 `soda_assets_manifest.json`；它记录相对路径、类型、推断类别、大小、修改时间和可探测的媒体元数据。路径、大小和修改时间未变化时不会重写；需要内容级变化检测时使用 `--checksum`。执行 Skill 的模型直接用 Read 工具查看原图和视频代表帧，为每条图片/视频补充中文 `description` 和源像素 `effective_region`。生成视频前，`preflight`/`render` 会阻止缺少 Manifest、asset_root 不一致、视觉素材缺少任一理解字段或时间轴引用未入库视觉素材的任务。
...
- `--script-file`：调用方提供口播文本时，作为字幕文案最高优先级；`render`/`repair-captions` 必须先对当前输入执行 Whisper `--word_timestamps True`，把台词填入真实词级时间戳后生成修正版时间轴，不覆盖原始时间轴；
- `materials`：只允许利益点物料；除路径、类型、布局和时间外，必须包含 `semantic_role=benefit_point` 与 `matched_benefit_text`；命中特殊规则时还必须包含合法的 `special_match_rule`，实际素材内容由执行模型 Read 确认，不使用 SHA-256 门禁；
- `motion_effects`：可选 Remotion 随机入场动效策略；缺省按 `mode=auto` 处理。字段和回退行为见 [motion-effects.md](motion-effects.md)；
- `visual_policy`：必须启用 `match_materials_only_for_benefit_points`、禁止背景承托层、要求字幕细描边且无阴影、强制 logo/警示语最高层，并要求有效内容区域避让品牌保护区；
...
`phone` 素材只做等比缩放和直接叠加，不绘制黑色背景板。素材无需避让字幕区域。渲染器从 Manifest 自动加载 `effective_region`，把它从源像素映射到实际布局，只用有效内容判断是否碰到 logo/警示语保护区；源画布、透明留白、纯色空白边距可以越界。有效内容碰撞时先移动整个素材，移动无法解决时才缩放整个素材。Remotion 使用 effective_region 裁切实际内容后生成入场层。所有素材完成叠加后，先绘制去标点 ASS 字幕与 CTA，再叠加 logo，最后绘制警示语。

处理新视频时复制模板 JSON 到任务目录，填写实际素材路径、字幕和时间点，不要直接覆盖 Skill 内的结构模板。模板中的占位值必须全部替换后才能预检或渲染。

按默认流程先使用 `trim-pauses` 生成去气口中间文件，再把该文件传给 `render`。若字幕和物料时间来自原视频，使用 `time_mode=original` 并填写实际 `removed_ranges`；若时间已经重算到中间文件，使用 `time_mode=input`。由 Whisper 生成的口播字幕逐条带 `time_mode=input`，即使其他素材仍使用全局 `time_mode=original`，renderer 也会分别按字幕自身的输入时间映射并在最终倍速后绘制。
