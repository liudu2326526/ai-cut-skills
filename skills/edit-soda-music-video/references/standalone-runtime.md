...
Manifest 默认写入工作区的 `soda_assets_manifest.json`；它记录相对路径、类型、推断类别、大小、修改时间和可探测的媒体元数据。路径、大小和修改时间未变化时不会重写；需要内容级变化检测时使用 `--checksum`。
...
- `visual_policy`：禁止渲染器生成背景黑条/黑框/半透明黑色承托层，要求字幕使用 `2–3px` 黑色细描边且阴影为 `0`，强制 logo 和警示语处于最高层，并要求源视频素材固定黑边检查按预检错误处理；
...
`phone` 素材只做等比缩放和直接叠加，不绘制黑色背景板。素材无需避让字幕区域；所有素材完成叠加后绘制 ASS 字幕与 CTA，再叠加 logo，最后绘制警示语。字幕允许 `2–3px` 黑色细描边，但不得绘制文字底板或阴影；警示语、数字人首屏钩子和 CTA 仍使用无背景的纯文字。logo 和警示语不得进入普通素材循环，否则全画布图片或手机录屏可能覆盖它们。
