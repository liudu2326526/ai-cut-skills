...
- `visual_policy`：禁止渲染器生成背景黑条/黑框/半透明黑色承托层，要求字幕使用 `2–3px` 黑色细描边且阴影为 `0`，并要求源视频素材固定黑边检查按预检错误处理；
...
`phone` 素材只做等比缩放和直接叠加，不绘制黑色背景板。素材无需避让字幕区域；所有素材完成叠加后再绘制 ASS 字幕和警示文字，确保文字位于素材上方。字幕允许 `2–3px` 黑色细描边，但不得绘制文字底板或阴影；警示语、数字人首屏钩子和 CTA 仍使用无背景的纯文字。
