...
- `materials` 的时间区间使用半开语义 `[start,end)`；同一 `sequence_id` 内上一条 `end` 必须严格等于下一条 `start`，禁止重叠和 0.2–0.3s 过渡缓冲；素材的起止点以及同序列切换点都必须对齐映射后字幕边界；有意返回数字人时使用不同 `sequence_id`。
...
`phone` 素材只做等比缩放和直接叠加，不绘制黑色背景板。`icon` 素材先按 Manifest `effective_region` 裁去空白画布，默认以有效内容源像素尺寸 `1:1` 叠加，不再执行 `scale=230:-1`；`x/y` 定位裁切后内容的左上角。素材无需避让字幕区域。渲染器只用有效内容判断是否碰到 logo/警示语保护区；源画布、透明留白、纯色空白边距可以越界。有效内容碰撞时先移动，移动无法解决时才缩放。Remotion 使用 effective_region 裁切实际内容后生成入场层。所有素材完成叠加后，先绘制去标点 ASS 字幕与 CTA，再叠加 logo，最后绘制警示语。
