...
素材无需避让字幕安全区，但必须避让 logo 和警示语保护区。1080×1920 默认素材可用区为 `x=48..1032, y=320..1740`，由 `visual_policy.material_safe_area` 的四边距控制。渲染器读取透明图片的 alpha 可见边界；可见内容越界时只裁出可见内容，再等比缩小并居中移入可用区。视频或无透明通道素材按完整画面等比放入可用区。Remotion 动效最终还要裁切到该可用区，避免入场放大或卷页过程侵入品牌区域。
