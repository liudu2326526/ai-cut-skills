...
- 金额档位、允许的利益点；
- 口播字幕文本和每个物料的入点/出点。
- 若调用方提供口播文件，确认其为字幕文本的权威来源；字幕时序仍必须来自对当前输入执行的 Whisper `--word_timestamps True`。移除普通标点，但保留数字内部小数点。
...
填写字体时必须使用字体文件的真实内部 family name，不能把文件名或正文字体名称复制给 SodaFont。字幕样式写入 `font.caption_style`；模板已把字号 13、缩放 100%、黑色细描边、阴影 0、`x=0,y=-500` 转为 1080×1920 ASS renderer 的校准配置：`outline=3`、`shadow=0`。1080 宽画布使用左右至少 `96px` 安全边距与 `0.92` 宽度余量，渲染器按中文、英文、数字和品牌字体的加权宽度显式均衡换行，优先一到两行、最多三行，不依赖 ASS 自动换行。不得拆分“汽水音乐”、小数金额或英文单词；三行仍无法容纳时，返回 Whisper 实际词级时间把长句拆成多个 caption，不得缩小字号。字幕文本去除普通中英文标点，但保留数字内部小数点；不得添加字幕背景条或半透明承托层。
...
使用 `qa` 检查分辨率、帧率、编码、时长、完整解码和响度；结合预检/渲染报告复核每条字幕的显式换行、行数和宽度，再抽取最长字幕帧确认左右没有越界。继续人工检查普通素材只出现在利益点、特殊利益点使用了符合画面要求的完整素材、“满三毛提现”没有误用局部截图或余额卡片、素材和动效的有效内容没有进入 logo/警示语保护区、空白画布没有导致无意义缩放、字幕位于素材上方、图标跟随最终字幕行数定位、logo 位于素材/字幕/CTA 上方、警示语处于最终最高层，字幕黑色细描边为 `2–3px`、阴影为 `0`、无背景黑条或承托层，以及黑屏/冻帧、logo 重叠、禁投歌曲、第三方标识和手机扬声器听感。
