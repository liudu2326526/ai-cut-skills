...
Remotion 只生成短透明 ProRes 4444 入场片段。物料稳定布局默认保持原尺寸，只有在确实碰到 logo/警示语保护区且平移无法解决时才按 `minimum_material_scale` 有限缩小；FFmpeg 合成时再把整个 Alpha 动效层裁切到素材可用区并透明补回全画布，防止放大、拖影、透视或卷页过程进入 logo 和警示语保护区。随后在动效素材上方绘制字幕与 CTA，再叠加 logo，最后绘制警示语。
