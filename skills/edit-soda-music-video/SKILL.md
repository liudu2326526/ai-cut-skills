...
`preflight` 和 `render` 都会检查 `--asset-manifest`（省略时默认为时间轴 JSON 所在目录的 `soda_assets_manifest.json`）：Manifest 必须与 `--asset-root` 一致，所有图片/视频必须有非空中文 `description` 和合法的源像素 `effective_region`，且时间轴引用的视觉素材必须已入库。每条 `materials[]` 还必须标记 `semantic_role=benefit_point` 和 `matched_benefit_text`；声明 `special_match_rule` 时只校验规则 ID 合法，不校验素材 SHA-256。执行模型必须通过 Read 确认实际画面满足特殊规则。门禁失败时先完成素材理解、利益点标注或特殊素材纠正，再重新预检。

素材策略写在时间线的 `visual_policy` 中：`match_materials_only_for_benefit_points=true`、`preserve_material_size=true`、`reposition_before_scale=true`。渲染器把 Manifest 的 `effective_region` 映射到画布，只在有效内容真实进入 logo/警示语保护区时处理；空白或透明画布越界不触发任何尺寸变化。有效内容碰撞时先移动整个素材，移动无法解决时才缩放整个素材到最大安全尺寸。
