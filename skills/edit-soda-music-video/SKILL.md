...
description: Create, revise, or validate Soda Music portrait talking-head mixed-cut videos with a standalone Python and FFmpeg workflow plus optional randomly selected Remotion entrance effects from an installed video-motion-effects skill. Includes mandatory Read-based understanding of every image/video before rendering, description-only semantic material matching by the executing model, Whisper tiny word timestamps that drive caller-script subtitle timing, punctuation-free captions, pause removal, speed remapping, logos, caller-supplied assets, compliance, BGM, end cards, export, and QA. Use for 汽水混剪、汽水音乐剪辑、素材内容理解、素材语义匹配、去气口、Whisper词级字幕、口播字幕校正、无标点字幕、口播加速、物料动效、随机入场特效、logo、素材清单、BGM、尾帧、渠道检查或自动化成片。
---
...
3. 生成任何视频前必须确认 Manifest 已覆盖当前素材根目录中的全部图片/视频，并且每条视觉素材都有执行模型通过 Read 实际查看后写入的中文 `description`。只要 Manifest 缺失、asset_root 不一致、存在空 description 或时间轴引用未入库视觉素材，先停止渲染，逐张 Read 图片和视频代表帧、补齐 Manifest，再重新运行预检；不得带着未理解素材进入 render。
4. 如果需要内容级素材匹配，执行模型在基础同步后直接用 Read 工具逐张查看所有新增、修改或缺少 description 的图片，并查看视频代表帧；将准确中文 description 写回 Manifest，再按口播语义选择真正匹配的素材。如果调用方提供 `--script-file` 口播，先对当前输入视频执行 Whisper 词级转写，再把口播台词按实际词级时间戳生成修正版字幕；原始时间轴不覆盖，修正版时间轴和修复报告单独输出。随后运行预检，检查素材理解门禁、字幕 `2–3px` 黑色细描边、阴影为 `0`、无背景承托层、素材不进入 logo/警示语保护区、素材默认尺寸未被改变、素材先平移，必要时再缩放到不遮挡品牌区域的最大尺寸、字幕高于素材、logo 高于素材/字幕/CTA、警示语处于最终最高层，以及源素材无固定黑边；不要在素材或运行环境缺失时猜测路径。
...
`preflight` 和 `render` 都会检查 `--asset-manifest`（省略时默认为时间轴 JSON 所在目录的 `soda_assets_manifest.json`）：Manifest 必须与 `--asset-root` 一致，所有图片/视频必须有非空中文 `description`，且时间轴引用的视觉素材必须已入库。门禁失败时先运行 `sync-assets`，由执行模型用 Read 逐张理解素材、写回 description，再重新预检。
