...
需要理解或检索素材时，阅读 [asset-content-understanding.md](references/asset-content-understanding.md)。执行 Skill 的模型必须直接用 Read 工具查看图片和视频缩略帧，为每个素材写入 `description` 和源像素坐标的 `effective_region`。先把口播分为利益点与非利益点，只有明确利益点允许匹配普通素材；寒暄、过渡、情绪铺垫、单独 CTA、免责声明等非利益点保持数字人主画面。不要调用素材理解/匹配脚本，也不要生成 `keywords`、`recommended_usage` 或向量索引。
...
3. 生成任何视频前必须确认 Manifest 已覆盖当前素材根目录中的全部图片/视频，并且每条视觉素材都有执行模型通过 Read 实际查看后写入的中文 `description` 和 `effective_region`。只要 Manifest 缺失、asset_root 不一致、存在空 description、缺失/无效有效区域或时间轴引用未入库视觉素材，先停止渲染，逐张 Read 图片和视频代表帧、补齐 Manifest，再重新运行预检；不得带着未理解素材进入 render。
...
素材内容理解和语义匹配不通过额外脚本执行。执行模型直接读取 Manifest、图片原图和视频代表帧，在 Manifest 素材记录中写入 `description` 与 `effective_region`；然后只针对口播中的利益点选择真实素材，并把利益点原文一并写入时间轴。详细操作见 [asset-content-understanding.md](references/asset-content-understanding.md)。
...
`preflight` 和 `render` 都会检查 `--asset-manifest`（省略时默认为时间轴 JSON 所在目录的 `soda_assets_manifest.json`）：Manifest 必须与 `--asset-root` 一致，所有图片/视频必须有非空中文 `description` 和合法的源像素 `effective_region`，且时间轴引用的视觉素材必须已入库。每条 `materials[]` 还必须标记 `semantic_role=benefit_point` 和 `matched_benefit_text`。门禁失败时先完成素材理解或利益点标注，再重新预检。
...
- QA 必须确认普通素材只出现在利益点台词时间段；字幕、警示语和所有素材外围不存在背景黑条、黑框或半透明黑色承托层；字幕文本与调用方口播一致且不含中英文标点；只检查素材有效内容与整个入场动效的有效内容是否进入 logo/警示语保护区，透明留白和无内容画布不得导致缩放；确有碰撞时先平移，再缩放到最大安全尺寸；字幕黑色细描边为 `2–3px`、阴影为 `0`，字幕始终位于素材上方，logo 位于素材/字幕/CTA 上方，警示语位于最终最高层；源素材固定黑边必须在预检阶段拦截。
