...
description: Create, revise, or validate Soda Music portrait talking-head mixed-cut videos with a standalone Python and FFmpeg workflow plus optional randomly selected Remotion entrance effects from an installed video-motion-effects skill. Includes description-only multimodal asset understanding and semantic material matching without a vector database, Whisper tiny pause timing, caller-script subtitle repair, punctuation-free captions, speed remapping, logos, caller-supplied assets, compliance, BGM, end cards, export, and QA. Use for 汽水混剪、汽水音乐剪辑、素材内容理解、素材语义匹配、去气口、口播字幕校正、无标点字幕、口播加速、物料动效、随机入场特效、logo、素材清单、BGM、尾帧、渠道检查或自动化成片。
---
...
需要大模型理解或检索素材时，阅读 [asset-content-understanding.md](references/asset-content-understanding.md)。内容理解只生成并保存一段 `content_understanding.description`；不要生成或保存 `keywords`、`recommended_usage` 或向量索引。
...
3. 如果需要内容级素材匹配，在基础同步后运行 `understand-assets`，只给图片和视频素材补充一段 description；如果调用方提供 `--script-file` 口播，先使用口播修正时间轴字幕文本并去除标点；原始时间轴不覆盖，修正版时间轴和修复报告单独输出。随后运行预检，检查字幕 `2–3px` 黑色细描边、阴影为 `0`、无背景承托层、素材不进入 logo/警示语保护区、素材默认尺寸未被改变、素材先平移，必要时再缩放到不遮挡品牌区域的最大尺寸、字幕高于素材、logo 高于素材/字幕/CTA、警示语处于最终最高层，以及源素材无固定黑边；不要在素材或运行环境缺失时猜测路径。
...
需要大模型理解图片/视频素材时，额外运行：

```bash
"$PY" "$PIPE" understand-assets \
  --manifest /absolute/path/workspace/soda_assets_manifest.json \
  --model <multimodal-model>
```

该步骤只在素材新增、内容指纹变化或 Prompt/模型版本变化时调用模型，并且只写入 `content_understanding.description`；不生成或保存 `keywords`、`recommended_usage` 或向量数据。模型配置也可以通过 `OPENAI_BASE_URL`、`OPENAI_API_KEY` 和 `OPENAI_MODEL` 提供。完整规则见 [asset-content-understanding.md](references/asset-content-understanding.md)。

根据口播检索已有素材时运行：

```bash
"$PY" "$PIPE" match-materials \
  --manifest /absolute/path/workspace/soda_assets_manifest.json \
  --query "介绍播放模式功能" \
...
匹配先做类型/分类等硬过滤，再根据素材 `description` 做文本预筛选和大模型排序；模型只能从已有素材路径中选择，确认后再写入时间轴 `materials[].path`。
