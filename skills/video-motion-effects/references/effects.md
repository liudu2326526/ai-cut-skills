...
| 5 | `page_curl` | `reference_fifth_v1` | 0..26 | `26/30s` |
...
来自原视频 F232..F258。使用单一透明 Canvas 绘制纵向网格，不使用多个 DOM 图片层，避免白卡出现竖向接缝。

- 默认 `slices=192`，允许 `64..256`；
- F232–F235 主要显示背面，F236–F238 分区切换正反面；
- F235–F238 使用实测的 7 点顶边/底边曲线；其余帧使用左、右、顶、底与卷曲量离散曲线；
- 第 20 参考帧后切换为单层矩形回正，防止低卷曲阶段重复采样纹；
- 默认时长 `26/30s`。
